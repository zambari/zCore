...
namespace Z
{
	...
	public class MoveRel : MonoBehaviour
	{
		...
		{
			get { return parentRect.rect.width; }
		}

		private float parentHeight
		{
			get { return parentRect.rect.height; }
		}
		...
		protected void OnValidate()
		{
			...
			if (setAnchors) rect.SetAnchorsX(0, 0);
			...
			if (parentRect == null)
				Debug.LogError("Please only use Move Rel on objects which parent have a Rect Transfrom ");
			if (previewSizeY)
			{
				previewnPositionX = false;
				previewnPositionY = false;
		...
		public float normalizedX
		{
			...
			set { rect.anchoredPosition = new Vector2(value * parentWidth, rect.anchoredPosition.y); }
		}
		...
		public float normalizedWidth
		{
			...
			set { rect.sizeDelta = new Vector2(value * parentWidth, rect.sizeDelta.y); }
		}

		public float normalizedY
		{
			get { return rect.anchoredPosition.y / parentWidth; }
			set { rect.anchoredPosition = new Vector2(rect.anchoredPosition.x, value * parentHeight); }
		}

		public float normalizedHeight
		{
			get { return rect.rect.height / parentHeight; }
			set { rect.sizeDelta = new Vector2(rect.sizeDelta.x, value * parentHeight); }
		}
