...
namespace ZUI
{
	public static class zExtensionsRect
	{
		...
		{
			if (parentToSet != null) rect.SetParent(parentToSet);
			var thisRect = rect.AddOrGetComponent<RectTransform>();
			if (parentToSet)
			{
		...
		public static RectTransform PadLeft(this RectTransform rect, float amount, bool additive = true)
		{
			...
			if (additive) offsetMin.x += amount;
			else offsetMin.x = amount;
		...
		{
			var offsetMax = rect.offsetMax;
			if (additive) offsetMax.x -= amount;
			else offsetMax.x = -amount;
			rect.offsetMax = offsetMax;
			return rect;
		}
		...
		{
			var offsetMin = rect.offsetMin;
			if (additive) offsetMin.y += amount;
			else offsetMin.y = amount;
			rect.offsetMin = offsetMin;
			return rect;
		}
		...
		{
			return rect.PadTop(amount, additive)
					   .PadBottom(amount, additive)
					   .PadRight(amount, additive)
					   .PadLeft(amount, additive);
		}
