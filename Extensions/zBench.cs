...
public static class zBench
{
	...
	private static Dictionary<string, float> _throttledStrings;

	private static Dictionary<string, List<float>> callDictionary;

	private const float throttledInterval = 15; // 5 seconds
...
#pragma warning restore 618
	...
	public static int EndMillis(string key, bool print = false)
	{
		...
		if (print)
		{
			if (sw.ElapsedMilliseconds < 5)
				Debug.Log(
					"Time between starting and finish of [" +
					key +
					"] was  " +
	...
	public static int EndMillis(string key, string message)
	{
		var sw = GetStopWatch(key);
		sw.Stop();
		Debug.Log(("[" + key + "] : " + sw.ElapsedMilliseconds + " ms  ") + message);
		stopwatchdict.Remove(key);
		return (int)sw.ElapsedMilliseconds;
	}
	...
	public static long EndTicks(string key, bool print = false)
	{
		...
		if (print)
		{
			if (sw.ElapsedMilliseconds < 5)
				Debug.Log(
					"Time between starting and finih of [" +
					key +
					"] was  " +
					sw.ElapsedMilliseconds +
					" ms (or " +
