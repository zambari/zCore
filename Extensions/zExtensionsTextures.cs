...
namespace Z
{
	...
	public static class zExtensionsTextures
	{
		...
		public static void Fill(this Texture2D texture, Color fillColor) //, bool apply=true
		{
			...
			for (int i = 0; i < black.Length; i++) black[i] = fillColor;
		...
		/// </summary>
		public static void FillStripes(
			this Texture2D texture,
			int x0,
			int y0,
			int x1,
			...
			Color color) //, bool apply=true
		{
			...
			for (int i = x0; i <= x1; i++)
				for (int j = y0; j <= y1; j++)
				{
					if (w < width) texture.SetPixel(i, j, color);
					w++;
					if (w > 2 * width) w = 0;
				}
		}
		...
		public static Texture2D FourCornerGradient(
			this Texture2D tex,
			Color color0,
			Color color1,
			Color color2,
		...
		public static RenderTexture CreateTestPattern(this RenderTexture rt)
		{
			...
			else texture = new Texture2D(rt.width, rt.height);

			int radius = Mathf.FloorToInt(Mathf.Min(texture.height, texture.width)) / 2;
			texture.Fill(Color.black);
			float colorMulti = 0.5f;
			texture.FourCornerGradient(
				Color.blue * colorMulti,
				Color.red * colorMulti,
				Color.green * colorMulti,
				Color.yellow * colorMulti);
		...
		public static Texture2D
			Create(this Texture2D t, Color fillColor, int sixeX = 1, int sizeY = 1) //, bool apply=true
		{
		...
		public static void Multiply(this Texture2D texture, Color fillColor) //, bool apply=true
		{
			...
			for (int i = 0; i < colors.Length; i++) colors[i] = colors[i] * fillColor;
