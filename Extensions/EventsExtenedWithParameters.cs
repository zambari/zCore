...
namespace Z
{
	...
	public class ComboBoolEvent
	{
		...
		private float lastTimeChange = -10;
		...
		public void Trigger(bool val)
		{
			...
			else
			{
				...
				else if (toggleState == ToggleStates.toggleOff)
				{
					toggleState = ToggleStates.toggleOffPendingOn;
					Debug.Log("state pending on " + Time.time);
				}
				else if (toggleState == ToggleStates.toggleOffPendingOn)
				{
					if (val == false) toggleState = ToggleStates.toggleOff;
				}
				else if (toggleState == ToggleStates.toggleOnPendingOff)
				{
